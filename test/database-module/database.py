...
counter = 0
filename = (d)
if (os.path.exists("file/"+filename+".xlsx") == False):
    filename = (filename)
else:
    while (os.path.exists("file/"+filename+"("+str(counter+1)+")"+".xlsx") == True):
        counter += 1
    filename = (filename+"("+str(counter+1)+")")
# print("\nThe file name is : "+filename)
f = xlsxwriter.Workbook("file/"+filename+'.xlsx')
fob = f.add_worksheet()
fob.write('A1', 'Name')
...
def enterData(z):
    ...
    if z in names:
        ...
        my_list = [z, branch.get()+'-'+batch.get(),
